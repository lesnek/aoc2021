...
pub fn solution2(data: Vec<String>) -> (u32, u32) {
    let bin_oxygen =
        u32::from_str_radix(filter_by_chem_type(data.clone(), "oxygen")[0].as_str(), 2).unwrap();

    let bin_co2 = u32::from_str_radix(filter_by_chem_type(data, "co2")[0].as_str(), 2).unwrap();
    (bin_oxygen, bin_co2)
}

fn filter_by_chem_type(mut data: Vec<String>, chem_type: &str) -> Vec<String> {
    let mut position = 0;
    while data.len() > 1 {
...
fn choose_data(data0: Vec<String>, data1: Vec<String>, chem_type: &str) -> Vec<String> {
    match chem_type {
        "oxygen" => {
        ...
        _ => panic!("Je to v pici, to se neda dychat ty vole!"),
    }
}
